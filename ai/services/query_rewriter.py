...
def _extract_last_document_name_from_sources(sources: Sequence[dict[str, Any]] | None) -> Optional[str]:
  ...
  name to use in rewriting (e.g. "BIP-12046.pdf"). Returns None if nothing usable.
  """
  if not sources:
      return None
  for src in sources:
...
def rewrite_with_last_document(question: str, last_sources: Sequence[dict[str, Any]] | None) -> str:
  ...
  # Hard length cap: do not rewrite long, complex questions in Phase B.1.
  if len(q) > 160:
      return original
  ...
  # Only even consider rewriting if we see a relevant pronoun/phrase.
  ...
  allow_it_rewrite = token_count <= 8
  ...
  rewritten = original
  for pat in patterns:
      # Be conservative with \"it\" to avoid nonsense rewrites in longer questions.
  ...
  if rewritten != original:
      logger.debug("Rewrote query using last document: '%s' → '%s' (doc=%s)", original, rewritten, last_doc_name)
  return rewritten
