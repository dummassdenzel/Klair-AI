...
**Location**: `ai/main.py:896-985` (`get_document_file`)

**Issue**: 
- File paths from database are used directly without validation against allowed directories
- No check if file is within the monitored directory
- Potential to access files outside intended scope

**Risk**: HIGH
- User could potentially access system files if database is compromised
- No directory boundary enforcement

**Current Code**:
```python
file_path = document.file_path  # From database, no validation
path_obj = Path(file_path)
if not path_obj.exists() or not path_obj.is_file():
    # Returns 404, but path could be outside allowed directory
```

**Fix Required**:
```python
# Validate file is within current_directory
...
**Similar Issue**: `get_document_preview` (line 987) has same vulnerability

---

### 2. No Input Validation on Directory Path
**Location**: `ai/main.py:224` (`set_directory`)

**Issue**:
- User-provided directory path is used directly
- No validation against path traversal (`../`, `..\\`)
- No check if directory is within allowed base path

**Risk**: HIGH
- Could monitor/process unintended directories
- Could access system directories

**Current Code**:
```python
directory_path = request.get("path")  # User input, no sanitization
directory_path = os.path.normpath(os.path.abspath(directory_path))  # Normalizes but doesn't restrict
```

**Fix Required**:
```python
# Add base directory restriction
ALLOWED_BASE_DIRS = [os.path.expanduser("~"), "C:\\Users"]  # Configurable
directory_path = request.get("path")
directory_path = os.path.normpath(os.path.abspath(directory_path))

# Validate it's within allowed base
if not any(directory_path.startswith(base) for base in ALLOWED_BASE_DIRS):
...
### 4. Missing File Size Limits on File Reading
**Location**: `ai/main.py:939` (`get_document_file` - TXT files)

**Issue**:
- TXT files are read entirely into memory without size check
...
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()  # Reads entire file, no size limit
```
...
# For very large files, consider streaming or truncation
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read(MAX_TEXT_FILE_SIZE)  # Limit read size
    if file_size > MAX_TEXT_FILE_SIZE:
...
**Location**: All API endpoints in `ai/main.py`

**Issue**:
- No rate limiting on expensive operations (chat, file processing)
- Vulnerable to DoS attacks
- Cost explosion risk (LLM API calls)
