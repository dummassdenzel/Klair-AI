...
| **Response generation speed** | **Retrieval**: Semantic search and BM25 run in parallel; classification and query embedding run in parallel. **Rerank**: Runs in `asyncio.to_thread`; config uses 20 docs for rerank. **Classification**: Domain-agnostic — LLM classifies all document intents; minimal greeting-only fast path (1–3 universal words); classification cache (normalized query → label, LRU 500) to reuse results and save latency on repeated/similar queries. **Post-response**: Document linking in parallel. **Streaming**: SSE for time-to-first-token. |
